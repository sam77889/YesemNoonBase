...
export function TaskCard({ task, source }: TaskCardProps) {
  ...
  return (
    <motion.div
      ...
      style={{ background: 'var(--surface-2)', padding: '1.25rem', borderRadius: '12px', border: '1px solid var(--border)' }}
    >
      <div className="flex justify-between items-center" style={{ marginBottom: '0.75rem' }}>
        <span style={{ fontWeight: 600, fontSize: '1rem' }}>
          [{task.status}] {task.status === 'SUCCESS' ? '✅ 任务完成: ' : task.status === 'FAILED' ? '❌ 任务失败: ' : '正在抓取: '}
          {task.query}
        </span>
